...
namespace Platformer
{
	...
	void Renderer::Frame::Clear(const Vector2& screenSize)
	{
		...
		for (int y = 0; y < height; ++y)
		{
			for (int x = 0; x < width; ++x)
			{
				...
				info.Char.AsciiChar = ' ';		// 빈 문자 설정 - 기존 설정된 값 지우는 효과
				info.Attributes = 0;			// 색상 표기 안함
	...
	{
		assert(!instance && "Renderer instance should be nullptr");
		instance = this;

		// 프레임 객세 생성
		const int bufferCount = screenSize.x * screenSize.y;
		frame = std::make_unique<Frame>(bufferCount);
		frame->Clear(screenSize);		// 생성 후 프레임 지우기
	...
	{
		assert(instance && "Renderer instance should not be nullptr");
		return *instance;
	}
	...
	void Renderer::DrawRenderQueue()
	{
		...
		for (const RenderCommand& command : renderQueue)
		{
			...
			// y 위치가 화면을 벗어났으면 건너뛰기 -> 액터가 1줄 짜리라는 전제 하에 진행
			if (command.position.y < 0 || command.position.y >= screenSize.y)
			{
				continue;
			}

			// 너비를 고려한 그리기 -> 그리려는 문자열 길이 값
