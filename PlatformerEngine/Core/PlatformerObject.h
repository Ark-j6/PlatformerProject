...
namespace Platformer
{
	...
	// 단일 상속만 지원
	class PLATFORMER_API PlatformerObject
	{
	public:
		// 가상 소멸자
		virtual ~PlatformerObject() = default;

		// 현재 객체 타입의 ID를 반환하는 ㅎ마수
		// 순수 가상함수로 만들기 -> 상속하는 계층에서 구현을 강제
		// C++에서 인터페이스를 사용할 대 이렇게 구성
		virtual size_t GetType() const = 0;

		// 전달된 다른 타입 ID와 현재 객체(부모 포함)의 타입 비교
		virtual bool Is(size_t id) const
		{
			// 최상위 객체인 CraftObject의 함수까지 접근헀다는 건 타입이 다르다는 것과 같은 의미
			return false;
		}
		...
		std::shared_ptr<T> Cast(const std::shared_ptr<U>& object)
		{
			...
			// object의 실제 타입이 T(또는 T의 파생) 타입인지 확인 후 형변환(캐스팅) 진행
...
public:																	\
	...
	{																	\
		...
		return (id == TypeIdClass()) ? true : super::Is(id);			\
	}